...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (id in conn.tebakgame) {
        return conn.reply(m.chat, '⚠️ Masih ada soal belum terjawab di chat ini!', conn.tebakgame[id][0])
    }
    
    await m.react('⏳')
    
    try {
        let res = await fetch('https://api.jagoanproject.com/api/game/tebakgame', {
...
handler.before = async function (m, { conn }) {
    ...
    if (!m.quoted || !m.text || !/TEBAK GAME/i.test(m.quoted.text)) return false
    if (!(id in conn.tebakgame)) return false
    ...
    if (m.quoted.id == conn.tebakgame[id][0].id) {
        ...
        if (m.text.toLowerCase().trim() === json.jawaban.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += conn.tebakgame[id][2]
            
            let txt = `╭── ⋆ ✧ ꒰ 🎀 *BENAR!* 🎀 ꒱ ✧ ⋆ ──\n`
            txt += `┊ 🎉 *Jawaban:* ${json.jawaban}\n`
            txt += `┊ 🎁 *Bonus:* +${conn.tebakgame[id][2]} XP\n`
            txt += `╰────────────────────── ⋆ ✧\n> 🌸 *Li Shiya MD - Game* 🌸`
