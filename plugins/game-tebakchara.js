...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (id in conn.tebakchara) {
        return conn.reply(m.chat, '⚠️ Masih ada soal belum terjawab di chat ini!', conn.tebakchara[id][0])
    }
    
    await m.react('⏳')
    
    try {
        let res = await fetch('https://api.jagoanproject.com/api/game/tebakchara?random=1', {
        ...
        let caption = `╭── ⋆ ✧ ꒰ 🎀 *TEBAK CHARA* 🎀 ꒱ ✧ ⋆ ──\n`
        caption += `┊ 🌸 *Soal:* Siapakah karakter anime ini?\n`
...
handler.before = async function (m, { conn }) {
    ...
    if (!m.quoted || !m.text || !/TEBAK CHARA/i.test(m.quoted.text)) return false
    if (!(id in conn.tebakchara)) return false
    ...
    if (m.quoted.id == conn.tebakchara[id][0].id) {
        ...
        if (m.text.toLowerCase().trim() === json.name.toLowerCase().trim()) {
            ...
            let txt = `╭── ⋆ ✧ ꒰ 🎀 *BENAR!* 🎀 ꒱ ✧ ⋆ ──\n`
            txt += `┊ 🎉 *Jawaban:* ${json.name}\n`
            txt += `┊ 🎁 *Bonus:* +${conn.tebakchara[id][2]} XP\n`
            txt += `╰────────────────────── ⋆ ✧\n> 🌸 *Li Shiya MD - Game* 🌸`
